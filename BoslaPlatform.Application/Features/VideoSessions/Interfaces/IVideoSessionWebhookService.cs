...
namespace BoslaPlatform.Application.Features.VideoSessions.Interfaces
{
    ...
    ///
    /// KEY RESPONSIBILITY — SOLE ACTIVATOR:
    ///   The ChannelCreated handler in this service is the ONLY code that transitions
    ///   a VideoSession from Waiting → Active. IVideoSessionService.StartAsync() is
    ///   a preparation/validation step only and does NOT activate the session.
    ///   This ensures the session's StartedAt timestamp always reflects the actual
    ///   moment the first participant joined (as reported by Agora), not the moment
