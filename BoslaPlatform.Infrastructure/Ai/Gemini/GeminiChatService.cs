...
public class GeminiChatService : IChatService
{
    ...
    public GeminiChatService(IOptions<GeminiSettings> options, GeminiHttpClient client, ILogger<GeminiChatService> logger)
    {
        ...
        public async Task<string> ChatAsync(string prompt, CancellationToken cancellationToken = default)
        {
            ...
            _logger.LogInformation("Gemini Chat Response: {ResponseJson}", doc.RootElement.GetRawText());

            // Gemini Interactions API response: { steps: [ { type: "model_output", content: [ { text: "..." } ] } ] }
            if (doc.RootElement.TryGetProperty("steps", out var steps) && steps.ValueKind == JsonValueKind.Array && steps.GetArrayLength() > 0)
            {
                for (int i = 0; i < steps.GetArrayLength(); i++)
                {
                    var step = steps[i];
                    if (step.TryGetProperty("type", out var stepType) && stepType.ValueKind == JsonValueKind.String)
                    {
                        var typeStr = stepType.GetString();
                        if (typeStr == "model_output" && step.TryGetProperty("content", out var stepContent) && stepContent.ValueKind == JsonValueKind.Array && stepContent.GetArrayLength() > 0)
                        {
                            var firstContent = stepContent[0];
            ...
            // Exact Gemini Interactions shapes (legacy)
            ...
            if (doc.RootElement.TryGetProperty("output", out var output) && output.ValueKind == JsonValueKind.Array && output.GetArrayLength() > 0)
            {
                ...
                if (firstOut.TryGetProperty("candidates", out var candidates) && candidates.ValueKind == JsonValueKind.Array && candidates.GetArrayLength() > 0)
                {
                    var c0 = candidates[0];
            ...
            if (doc.RootElement.TryGetProperty("candidates", out var candRoot) && candRoot.ValueKind == JsonValueKind.Array && candRoot.GetArrayLength() > 0)
            {
                var c0 = candRoot[0];
